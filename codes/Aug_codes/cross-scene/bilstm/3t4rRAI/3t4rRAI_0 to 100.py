...
x_test_new = np.load(h5_dir + 'x_train.npy')
y_test_new = np.load(h5_dir + 'y_train.npy') 

#x_test_blank = np.load(blank_dir + 'x_blank.npy')

x_train_new = np.load(h5_dir + 'x_test.npy')
y_train_new = np.load(h5_dir + 'y_test.npy') 
#print (x_test.shape)
#print (y_test.shape) 
...
#for i in range(1, 1440): #train as 80%
##    if i % 10 == 9 or i % 10 == 8 or i % 10 == 7 or i % 10 == 6 or i % 10 == 5:
##    if i % 10 == 9 or i % 10 == 8 or i % 10 == 7 or i % 10 == 6:
##    if i % 10 == 9 or i % 10 == 8 or i % 10 == 7:
#    
...
##    if i % 10 == 9:
#        x_train_new = np.concatenate((x_train_new, x_train[np.newaxis,i,...]), axis=0)
#        y_train_new = np.concatenate((y_train_new, y_train[np.newaxis,i,...]), axis=0)
##        x_train_new = np.delete(x_train_new, i, 0)
##        y_train_new = np.delete(y_train_new, i, 0)
#        
#for i in range(9, 1440): #test as 20%
##    if i % 10 == 0 or i % 10 == 1 or i % 10 == 2 or i % 10 == 3 or i % 10 == 4 or i % 10 == 5 or i % 10 == 6 or i % 10 == 7: 
#    if i % 10 == 8 or i % 10 == 9:
#        x_test_new = np.concatenate((x_test_new, x_test[np.newaxis,i,...]), axis=0)
#        y_test_new = np.concatenate((y_test_new, y_test[np.newaxis,i,...]), axis=0)
...
output_dir = '/home/lab210/MattYen_workspace/gest_August/cross-scene/NEW/bilstm/3t4rRAI/0-100/'
#output_dir = h5_dir
...
#model = load_model(output_dir + 'model_NewConfig_bilstm_3t4rRAI_32x32_Gaussian_batch12_ProgressSGD.h5')
#model.load_weights(output_dir + 'weightsNewConfig_bilstm_3t4rRAI_32x32_Gaussian_batch12_ProgressSGD.h5', by_name = True)
#score = model.evaluate(x_test_new, y_test_new, batch_size=12)
#predictions = model.predict(x_test_new) 
