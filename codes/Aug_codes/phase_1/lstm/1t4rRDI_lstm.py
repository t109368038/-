...
h5_dir='D:/real-time-radar/gest_August/cross-scene/3t4rRDI/'
...
#=========================================================================#
x_train_original = np.load(h5_dir + 'x_train.npy')
y_train_original = np.load(h5_dir + 'y_train.npy') 

#x_test_blank = np.load(blank_dir + 'x_blank.npy')

x_test_original = np.load(h5_dir + 'x_test.npy')
y_test_original = np.load(h5_dir + 'y_test.npy') 
#print (x_test.shape)
#print (y_test.shape) 

x_train_original = np.transpose(x_train_original, (0,1,3,4,2))
x_test_original = np.transpose(x_test_original, (0,1,3,4,2))
...
#    if i % 10 == 9:
        x_train[i,...] = x_test_original[i,...]
        y_train[i,...] = y_test_original[i,...]
        x_test[i,...] = x_train_original[i,...]
        y_test[i,...] = y_train_original[i,...]        

        
del x_train_original, y_train_original, x_test_original, y_test_original
...
x_test = (x_test - x_train_mean_perChannel)/x_train_std
...
for i in range(0, 12):
    for j in range(0, 12):
        for k in range(0, 10):
            mat_percentage_per_frame[j, :] += np.sum(frame_prob_tmp[count_1, :, :], axis=0)
            count_1 += 1
...
# output_dir = '/home/lab210/MattYen_workspace/gest_August/original_cases/phase_1/1t4rRDI/lstm/'
#output_dir = h5_dir
...
y_test_original = np.reshape(y_test_original, [-1, 12])
y_cat = np.argmax(y_test_original, axis=1)

prediction = np.reshape(predictions, [-1, 12])
...
# model.save(output_dir + 'model_NewConfig_lstm_1t4rRDI_32x32_Gaussian_batch12_ProgressSGD.h5')
# model.save_weights(output_dir + 'weights_NewConfig_lstm_1t4rRDI_32x32_Gaussian_batch12_ProgressSGD.h5')