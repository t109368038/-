...
h5_dir='/home/pstudent/DeepSoli Models/gesture_data/Range-Angle_fair comparison/final data/1t4r_RAI/'
model_dir = '/home/pstudent/DeepSoli Models/gesture_data/Range-Angle_fair comparison/final data/1t4r_RAI/whole set as train/'
vali_dir='/home/pstudent/DeepSoli Models/gesture_data/Range-Angle_fair comparison/final data/3t4r_RAI/whole set as train/lab210_2/'
...
#x_train = (x_train > -0.20) * x_train
#x_test = (x_test > -0.20) * x_test


#==============================================================================
model = load_model(model_dir + 'model_3t4r_bilstm_RangeDoppler_32x32_gaussed_batch12_ProgressSgd.h5')
model.load_weights(model_dir + 'weights_3t4r_bilstm_RangeDoppler_32x32_gaussed_batch12_ProgressSgd.h5', by_name = True)
#score = model.evaluate(x_test, y_test, batch_size=12)
predictions = model.predict(x_test) 

#==============================================================================
#x_test_4r_1 = x_test[:,:,:,:,0] + x_test[:,:,:,:,4] + x_test[:,:,:,:,8]
#x_test_4r_2 = x_test[:,:,:,:,1] + x_test[:,:,:,:,5] + x_test[:,:,:,:,9]
#x_test_4r_3 = x_test[:,:,:,:,2] + x_test[:,:,:,:,6] + x_test[:,:,:,:,10]
#x_test_4r_4 = x_test[:,:,:,:,3] + x_test[:,:,:,:,7] + x_test[:,:,:,:,11]
#
#x_test = np.concatenate((x_test_4r_1[..., np.newaxis], 
#                         x_test_4r_2[..., np.newaxis], 
#                         x_test_4r_3[..., np.newaxis], 
#                         x_test_4r_4[..., np.newaxis]), axis=4)
#x_test = x_test/3
#np.save('x_test.npy', x_test)
#np.save('y_test.npy', y_test)

#x_test_3t_sum = x_test[:,:,:,:,0] + x_test[:,:,:,:,1] + x_test[:,:,:,:,2]
#
#x_test = x_test_3t_sum[..., np.newaxis]
#x_test = x_test/3
#np.save('x_test.npy', x_test)
#np.save('y_test.npy', y_test)
...
#
#x_train = (x_train - x_train_mean_perChannel)/x_train_std
...
count=0;
for j in range(0, 7):      
    temp = sum(frames_prob_sum[count:count+168,:])
    mat_percentage[j, :] = temp/168
    count+=168     

for j in range(7, 10):
    temp = sum(frames_prob_sum[count:count+96,:])
    mat_percentage[j, :] = temp/96
    count+=96
...
#fig_acc.savefig(output_dir +'acc.jpg')
...
model.save(output_dir + 'model_1t4r_lstm_RangeAngle_32x32_gaussed_batch12_ProgressSgd.h5')
model.save_weights(output_dir + 'weights_1t4r_lstm_RangeAngle_32x32_gaussed_batch12_ProgressSgd.h5')
