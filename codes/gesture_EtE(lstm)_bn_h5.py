...
def RNN(x, batch_num, n_hidden, output_size):
    ...
    output, states = tf.nn.dynamic_rnn(lstm_cell, x, initial_state=init_state,
                                                     time_major=False)    

    # Linear activation, using rnn inner loop last output
    return tf.reshape(tf.concat(output,1), [-1, n_hidden])

x_image = tf.reshape(xs, [-1, 64, 32, channel])
...
h_conv2 = tf.nn.dropout(h_conv2, rate = conv2_dropout)
...
h_conv3 = tf.nn.dropout(h_conv3, rate = conv3_dropout)

# [n_samples, 58, 26, 128] ->> [n_samples, 58*26*128]
h_flat = tf.reshape(h_conv3, [-1, 58*26*128])

## fc4 layer ##
W_fc4 = weight_variable([58*26*128, 512])
b_fc4 = bias_variable([512])
h_fc4 = tf.nn.relu(tf.matmul(h_flat, W_fc4) + b_fc4)
h_fc4_drop = tf.nn.dropout(h_fc4, rate = fc4_dropout)

## fc5 layer ##
W_fc5 = weight_variable([512, 512])
b_fc5 = bias_variable([512])
h_fc5 = tf.nn.relu(tf.matmul(h_fc4_drop, W_fc5) + b_fc5)
h_fc5_drop = tf.nn.dropout(h_fc5, rate = fc5_dropout)

h_fc5_reshaped = tf.reshape(h_fc5_drop, [-1, seq_len, 512])
h_lstm6 = RNN(h_fc5_reshaped, batch_num, n_hidden=512, output_size=512)

W_fc7 = weight_variable([512, n_classes])
b_fc7 = bias_variable([n_classes])
h_fc7 = tf.matmul(h_lstm6, W_fc7) + b_fc7
y_pred = tf.nn.dropout(h_fc7, rate = fc7_dropout)
...
with tf.Session() as sess:
    ...
    result_txt = open('gesture_lstm_0522_bn_non-normed.txt' ,'w')
    sess.run(init_op)
