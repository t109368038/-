...
class Realtime_sys():
    ...
    def pd2voxel(self,data):
        ...
        for c in range(tmp_frame.shape[0]):
            ...
            if x < 25 and y < 25 and z < 25 and x > 0 and y > 0 and z > 0:
                ...
                tz += tmp_frame[c][2]
        if point_count !=0:
            out_radar_p = [tx/point_count,ty/point_count,tz/point_count]
        else:
            out_radar_p = [0, 0, 0]
        return out_radar_p
    ...
    def Qthreadupdate_fig(self,rdi,rai,pd):
        ...
        tmp_pd = pd.T
        tmp_out = self.pd2voxel(tmp_pd)
        self.x.append(tmp_out[0]*-1)
        self.y.append(tmp_out[1]*-1)
        self.z.append(tmp_out[2])
        # tmp_xy = np.ndarray([self.x[:], self.y[:]])

        self.xy_plane.setData(self.x,self.y, pxMode=True)
        self.xz_plane.setData(self.x ,self.z, pxMode=True)
        if len(self.x) > 10:
            self.x = self.x[1:]
            self.y = self.y[1:]
            self.z = self.z[1:]
