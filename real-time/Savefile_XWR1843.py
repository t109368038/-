...
# -----------------------------------------------
from app.app_savefile import Ui_MainWindow
# -----------------------------------------------
...
def save_test():
    global MainWindow,save_text,suresave
    suresave = 1
    print(save_text.toPlainText())

def update_figure():
    global img_rdi, img_rai, updateTime , number ,save_data ,tt,MainWindow,save_text,suresave
    while True:
        try:
            ...
            updateTime = now
            if suresave:
                save_data = BinData.get()
                txt = save_text.toPlainText()
                path = "C:/Users/user/Desktop/data/"
                savefile = path + txt + '.npy'
                print(savefile)

                np.save(savefile, save_data)
                save_data = []
                suresave = 0
            # if number < 64:
            #     save_data.append(BinData.get())
            #     number += 1
            # else:
            #     np.save("C:/Users/user/Desktop/1t4r11211.npy", save_data)
            #     tt.StopRadar()
            #     sys.exit()
...
def plot(cfg):
    global img_rdi, img_rai, updateTime,MainWindow,save_text
    #---------------------------------------------------
    ...
    save_btn = ui.pushButton_save
    save_text = ui.textEdit
    #---------------------------------------------------
    ...
    save_btn.clicked.connect(save_test)
